...
same primitives to mjlab's pelvis-root G1. The latter deliberately preserves GMTrack's
...
def hip_pitch_effort_limit() -> float:
  ...
  Override with ``GMTRACK_HIP_PITCH_EFFORT`` (N*m); see
  :data:`SONIC_HIP_PITCH_EFFORT_LIMIT` for what it does and does not change.
  """
  raw = os.environ.get("GMTRACK_HIP_PITCH_EFFORT")
  if raw is None:
    return MJLAB_HIP_PITCH_EFFORT_LIMIT
  value = float(raw)
  if value <= 0.0:
    raise ValueError(f"GMTRACK_HIP_PITCH_EFFORT must be positive, got {value}.")
  return value
...
def get_gmtrack_g1_robot_cfg() -> EntityCfg:
  """Return the GMTrack G1 while preserving mjlab's dynamics and joint order."""
