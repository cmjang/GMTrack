...
def main(cfg: Config) -> None:
  ...
  # Reuse the exact robot asset used by GMTrack training, including its InstinctMJ
  ...
  scene_cfg.entities["robot"] = get_gmtrack_g1_robot_cfg()
  ...
  try:
    for file_index, csv_path in enumerate(
      ...
      try:
        ...
        tqdm.write(f"[gmtrack] skipped invalid source {relative}: {exc}")
        continue
  ...
  print(
    f"[gmtrack] wrote {len(clips)} complete sequences "
