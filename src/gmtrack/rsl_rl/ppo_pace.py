...
* *Valid sample* = a rollout step at or before the environment's first true tracking
  ...
  :meth:`~gmtrack.rsl_rl.storage.StarRolloutStorage.valid_sample_counts`.
...
from gmtrack.rsl_rl.models import GMTrackActor
from gmtrack.rsl_rl.storage import TRACKING_FAILURES_EXTRA, StarRolloutStorage
...
def _runtime_observation_schema(
  ...
  if not isinstance(command, MultiMotionCommand):
    raise TypeError("GMTrack observation schema requires MultiMotionCommand.")
  ...
  if (
    configured_actor_offsets is not None
    and list(configured_actor_offsets) != runtime_actor_offsets
  ):
    raise ValueError(
      "Actor positional-encoding offsets do not match the command window: "
      f"{list(configured_actor_offsets)} != {runtime_actor_offsets}."
    )
  configured_reconstruction_offsets = list(actor_cfg["future_reconstruction_offsets"])
  ...
  return build_observation_schema(
    ...
    proprio_term_names=list(raw_env.observation_manager.active_terms["proprio_hist"]),
    ...
    use_intent_in_actor=bool(actor_cfg.get("use_intent_in_actor", False)),
  )
...
class PacePPO(PPO):
  ...
  def __init__(
    ...
    **kwargs,
    ...
    if intent_reconstruction_coef < 0.0 or intent_kl_coef < 0.0:
      raise ValueError(
        "Intent reconstruction and KL coefficients must be non-negative."
      )
    self.intent_reconstruction_coef = intent_reconstruction_coef
    self.intent_kl_coef = intent_kl_coef
    self.use_intent_aux = intent_reconstruction_coef > 0.0 or intent_kl_coef > 0.0
    if self.use_intent_aux:
      if not isinstance(self._raw_actor, GMTrackActor):
        raise TypeError("Intent auxiliary losses require GMTrackActor.")
  ...
  def save(self) -> dict:
    ...
    saved[CHECKPOINT_OBSERVATION_SCHEMA_KEY] = copy.deepcopy(self.observation_schema)
    if self.actor_ref is not None:
  ...
  def construct_algorithm(  # type: ignore[override]
    ...
    command = raw_env.command_manager.get_term("motion")
    if not isinstance(command, MultiMotionCommand):
      raise TypeError("GMTrack checkpoint schema requires MultiMotionCommand.")
    ...
    if base_checkpoint:
      ...
      print(f"[gmtrack] PACE: warm-started from {base_checkpoint}")
