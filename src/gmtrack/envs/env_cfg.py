...
from gmtrack import mdp
from gmtrack.assets import get_gmtrack_g1_robot_cfg
from gmtrack.mdp.actions import ReferenceResidualJointPositionActionCfg
from gmtrack.mdp.commands import MultiMotionCommandCfg
...
def _proprio_terms(
  ...
  ORDER IS LOAD-BEARING: ``GMTrackActorCfg.proprio_term_dims`` must match it, because
...
def make_gmtrack_env_cfg(
  ...
  observations = {
    ...
    # Not a policy input -- see gmtrack.mdp.observations.motion_star_meta.
  ...
  actions: dict[str, ActionTermCfg] = {
    ...
    # gmtrack.mdp.actions for why the difference matters.
  ...
  commands: dict[str, CommandTermCfg] = {
    "motion": MultiMotionCommandCfg(
      ...
      critic_window_offsets=(CAUSAL_CRITIC_WINDOW_OFFSETS if causal_online else None),
  ...
  if sonic_foot_terminations:
    # Split SONIC's foot guard by axis so horizontal landing placement and foot
    # height have their own thresholds and diagnostics. ``body_pos_relative_w`` has
    # already re-anchored the reference to the robot pelvis XY and yaw.
    terminations["foot_pos_xy"] = TerminationTermCfg(
      func=mdp.bad_motion_body_pos_xy_outside_recovery,
      params={
        "command_name": "motion",
        "threshold": 0.2,
    ...
    terminations["foot_pos_z"] = TerminationTermCfg(
      func=mdp.bad_motion_body_pos_z_only_outside_recovery,
      params={
        "command_name": "motion",
        "threshold": 0.15,
  ...
  cfg = ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      entities={"robot": get_gmtrack_g1_robot_cfg()},
