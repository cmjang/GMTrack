...
* A :class:`~gmtrack.mdp.motion_library.MotionLibrary` replaces the single
...
class MultiMotionCommand(CommandTerm):
  ...
  def __init__(self, cfg: MultiMotionCommandCfg, env: ManagerBasedRlEnv):
    ...
    if cfg.require_v1_stratification:
      ...
      from gmtrack.protocol import validate_stage2_manifests
    ...
    if cfg.recovery_probability > 0.0:
      # ``make_gmtrack_env_cfg`` only *builds* the assistance-force event term when it
      ...
      print(
        "[gmtrack] RGMT random-pose recovery: "
    ...
    # ``GMTrackOnPolicyRunner`` so the anneal survives a restart of a recovery run.
  ...
  def _setup_roles(self) -> None:
    ...
    print(
      f"[gmtrack] PACE roles: acquisition={self.acq_env_ids.numel()} envs over "
