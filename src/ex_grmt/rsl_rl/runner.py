...
class ExGRMTOnPolicyRunner(MjlabOnPolicyRunner):
  ...
  def _write_training_provenance(self, log_dir: str, train_cfg: dict) -> None:
    ...
    command = self._motion_command()
    if command is None:
      raise ValueError("ExGRMT training provenance requires a motion command.")
    cfg = command.cfg
  ...
  def save(self, path: str, infos=None) -> None:
    infos = {**(infos or {}), "ex_grmt_env_state": self._collect_env_state()}
  ...
  def load(
    ...
    """Restore the model plus ExGRMT's adaptive environment state.
    ...
    if restore_env_state and infos and "ex_grmt_env_state" in infos:
      self._restore_env_state(infos["ex_grmt_env_state"])
    return infos

  def _motion_command(self) -> MultiMotionCommand | None:
    """Return the motion term when this runner owns an ExGRMT environment."""
  ...
  def _restore_env_state(self, state: dict[str, Any]) -> None:
    ...
    if version not in (1, 2):
      raise ValueError(f"Unsupported ExGRMT environment state version {version!r}.")
    command = self._motion_command()
    if command is None:
      return
    ...
    if "samplers" not in state:
      raise ValueError(
        "ExGRMT environment state has no 'samplers' entry, but this environment has "
    ...
    if not isinstance(saved_samplers, dict):
      raise ValueError("ExGRMT environment state 'samplers' entry must be a mapping.")
    expected_names = {
      name
      for name in ("sampler_acq", "sampler_con")
    ...
    if version == 1:
      ...
      print(
        "[ex-grmt] checkpoint predates the recovery-local assist anneal; "
        "restarting the anneal clock at 0."
      )
    elif "recovery_steps_elapsed" not in state:
      raise ValueError(
        "Version-2 ExGRMT environment state is missing 'recovery_steps_elapsed'."
      )
    else:
