...
body ordering and all velocities using the current G1 model::
  ...
  python -m ex_grmt.scripts.curate_seed_simple curate
  python -m ex_grmt.scripts.prepare_motions \
  ...
  python -m ex_grmt.scripts.curate_seed_simple merge
"""
...
def curate(args: argparse.Namespace) -> None:
  ...
  print(
    f"[ex-grmt] selected {len(selected)} mirrored pairs / {len(report_entries)} "
    f"clips; expected converted duration "
    f"{payload['selection']['expected_converted_hours']:.4f} h"
  )
  print(f"[ex-grmt] pair quotas: {dict(counts)}")
  print(f"[ex-grmt] CSVs: {output_dir}")
  print(f"[ex-grmt] report: {report_path}")
...
def merge(args: argparse.Namespace) -> None:
  ...
  print(
    f"[ex-grmt] merged {len(clips)} clips / {duration:.4f} h to {output_path}; "
