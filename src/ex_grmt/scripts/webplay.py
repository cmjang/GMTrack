"""Launch a Viser web viewer for a local Ex-GRMT checkpoint and manifest.
...
class ExGRMTWebplayViewer(ViserPlayViewer):
...
def main(cfg: Config) -> None:
  ...
  server = viser.ViserServer(port=cfg.port, label="Ex-GRMT")
  try:
    ExGRMTWebplayViewer(env, policy, viser_server=server).run()
  finally:
    env.close()
    server.stop()
