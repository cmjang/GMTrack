...
def strip_failure_terminations(env_cfg) -> None:
  ...
  z-deviation check) trip well before that criterion and would deflate Succ. below
  anything comparable to Table VI. Sequence completion is owned by the rollout
  harness as an exact ``N - 1`` transition horizon: retaining that timeout would let
  the environment reset before its terminal state is measured. ``del`` raises if a
  name is ever missing, so a renamed termination cannot silently enter evaluation.
...
def _configure_random_recovery_rollout(env_cfg, *, nominal: bool) -> None:
  ...
  if "recovery_assist" not in env_cfg.events:
    raise ValueError(
      ...
      "enabled, such as 'ExGRMT-Stage1-Recovery-Flat-Unitree-G1'."
    )
...
def _inference_runner_cfg(agent_cfg) -> dict:
  """Build a runner config without Stage-II's training-only ``pi_ref`` dependency."""
